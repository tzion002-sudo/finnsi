...
export default function HaMatzpanGemelnet() {
  ...
  // V2.9.3 — Channel Alerts (scanner_alerts/latest from Firestore)
  ...
  // V2.9.3 — Channel Alerts real-time listener
  useEffect(() => {
    const unsub = subscribeToAlerts(data => {
      setChannelAlerts(data?.alerts || []);
    });
    return () => { try { unsub?.(); } catch {} };
  }, []);
    ...
    <div dir="rtl" className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8" style={{fontFamily:"system-ui, -apple-system, Segoe UI, sans-serif"}}>
      ...
      <div className="flex items-center gap-1 mb-5 border-b border-slate-700 overflow-x-auto">
        {[
          ...
          { key:"alerts",      label:"🚨 התראות",   icon:<Bell size={14}/>,             color:"border-red-500",     badge: channelAlerts.length || null },
        ].map(t => (
